...
#' @description The function implements the Solar Position Algorithm described in Ibrahim R and Afshin A, (2008)
#' technical report.
#' @details
...
#' @examples {
#' 
#'   spa(year=2003,month=10,day=17,hour=12,minute=30,sec=30, timezone=-7)
#' 
#' }
#'
spa <- function (year, month, day, hour = 12, minute = 0, sec = 0, timezone=0, longitude=-105.1786, latitude=39.742476, elevation=1830.14, pressure=820, temperature=11, surface.slope=30, surface.azimuth.rotation=-10, DeltaT=67){
...
if (JD.UT0 > 2299160){
  A <- trunc(Y.UT0/100)
  JD.UT0 <- JD.UT0 + (2 - A + trunc(A/4))
}

# Calculate the Julian Ephemeris Day (JDE.UT0)
JDE.UT0 <- JD.UT0 + DeltaT/86400
...
# Ibrahim and Afshin 2008 Report.
#
###############################################################################
...
m0 <- (alpha0 - sigma - v)/360

# Calculate the local hour angle corresponding to the sun elevation (h0Prime=-0.8333)
h0Prime <- -0.8333
arqument <- (sin(h0Prime*pi/180)-sin(phi*pi/180)*sin(delta0*pi/180))/(cos(phi*pi/180)*cos(delta0*pi/180))
if (abs(arqument) <=1){
  H0 <- acos(arqument)
}
else{
...
# Calculate the sidereal time at Greenwich, in degrees, for the sun transit, sunrise and sunset, vi
v0 <- v + 360.985647*m0
v1 <- v + 360.985647*m1
v2 <- v + 360.985647*m2

# Calculate the terms ni,
n0 <- m0 + DeltaT/86400
n1 <- m1 + DeltaT/86400
n2 <- m2 + DeltaT/86400

return(list(JD=JD,L0=L0,L1=L1,L2=L2,L3=L3,L4=L4,L5=L5,L=L.limited,B0=B0,B1=B1,B=B,R0=R0,R1=R1,R2=R2,R3=R3,R4=R4,R=R,Theta=Theta.limited,beta=beta,Deltapsi=Deltapsi,Deltaepsilon=Deltaepsilon,epsilon=epsilon,lambda=lambda,alpha=alpha.limited,delta=delta,H=H.limited,alphaPrime=alphaPrime,deltaPrime=deltaPrime,HPrime=HPrime,theta=theta,Phi=Phi.limited,I=I,M=M.limited,E=E))
